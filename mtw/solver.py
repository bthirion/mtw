...
def _update_coefs(pll, X, y, Ls, marginals, sigmas, a, b, sigma0,
                 coefs0=None, R=None, maxiter=20000, tol=1e-4,
                 positive=False):
    """BCD in numba."""
    n_tasks, n_samples, n_features = Y.shape
    n_samples, n_components = X.shape
    n_features = Ls.shape[1] ###

    if coefs0 is None:
        R = y.copy()
        coefs0 = np.zeros((n_features, n_components, n_tasks))
    elif R is None:
        R = utils.residual(X, coefs0, y)
...
def _solver(X, Y, M, alpha=1., beta=0., epsilon=0.01, gamma=1., sigma0=0.,
           stable=False, maxiter=2000, callback=None, tol=1e-5,
           maxiter_ot=10000, tol_ot=1e-4, positive=False, n_jobs=1,
           tol_cd=1e-4, gpu=True, verbosxe=True):
    """Perform Alternating Optimization of concomittant CRW."""
    ...
    coefs01 = np.ones((n_features, n_components, n_tasks)) ###
    coefs02 = np.ones((n_features, n_components, n_tasks)) ###
    marginals1 = np.ones((n_tasks, n_components, n_features)) / n_features
    marginals2 = np.ones((n_tasks, n_components, n_features)) / n_features

    Xf = [np.asfortranarray(np.eye(n_features)) for k in range(n_tasks)]
    mXf = [- np.asfortranarray(np.eye(n_features)) for k in range(n_tasks)]
    ...
    Y_ = np.array([np.linalg.pinv(X[k]).dot(Y[k]) for k in range(n_tasks)])

    Theta1, Theta2, Thetabar1, Thetabar2, Log, Sigmas = [] * 6
    for comp in range(n_components):
        with Parallel(n_jobs=n_jobs, backend="threading") as pll:
            if alpha == 0.: ### no Wasserstein
                t = time()
                theta1 = np.asfortranarray(theta1)
                theta, R, sigmas = _update_coefs(pll, Xf, Y_[comp], Ls, marginals1,
                                                 sigmas,
                                                 a, b,
                                                 sigma0,
                                                 coefs0=theta1,
            ...
            for i in range(maxiter):
                t0 = time()
                theta1, R, sigmas = _update_coefs(pll, Xf, Y_[comp], Ls, marginals1,
                                                 sigmas,
                                                 a, b,
                                                 sigma0,
                                                 coefs0=theta1,
        ...
        Thetabar1.append(thetabar1)
        Thetabar2.append(thetabar2)
        Log.append(log)
        Sigmas.append(sigmas)
        
    Theta1 = np.array(Theta1)
    Theta2 = np.array(Theta2)
    Thetabar1 = np.array(Thetabar1)
    Thetabar2 = np.array(Thetabar2)
    return Theta1, Theta2, Thetabar1, Thetabar2, Log, Sigmas
